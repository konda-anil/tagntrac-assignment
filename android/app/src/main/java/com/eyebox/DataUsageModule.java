...
import com.eyebox.helper.NetworkStatsHelper;
...
public class DataUsageModule extends ReactContextBaseJavaModule {
    ...
    private JSONObject getNetworkManagerStats(int uid, String name, String packageName, String encodedImage, Date startDate, Date endDate) {
        ...
        // long wifiBytesRx =  networkStatsHelper.getPackageRxBytesWifi(startDate, endDate);
        // long wifiBytesTx =  networkStatsHelper.getPackageTxBytesWifi(startDate, endDate);
        // long total = wifiBytesRx + wifiBytesTx;

        long total = networkStatsHelper.getPackageBytesWifi(startDate, endDate);
        ...
        try {
            ...
            if(total > 0L) {
                return new JSONObject().put("appId", uid)
                                    .put("appName", name)
                                    .put("appNamePackageName", packageName)
                                    .put("appDataUsage", total)
                                    .put("appIcon", encodedImage);
            }
        } catch (JSONException e) {
